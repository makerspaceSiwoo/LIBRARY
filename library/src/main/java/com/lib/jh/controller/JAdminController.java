...
public class JAdminController {
	...
	// 로그인 기능 생기면 변환하기 
	    ...
	    //user.ban에 현재시간 + 3일 값을 넘긴다
	    @PostMapping("/admin/ban/forbid_end")
	    public String adminBanForbid_end(@RequestParam("blacklistno")int blacklistno,@RequestParam("userno") int userno, @RequestParam("boardno") int boardno) {
	    	 // 현재 날짜를 Date 객체로 가져옴
